...
iniglobals(int calling_pid)
{
		...
		writers = 0;
		pending_writers = 0;
		now.readers_proceed.x = 0;
		now.writer_proceed.x = 0;
#ifdef VAR_RANGES
		logval("readers_proceed.x", now.readers_proceed.x);
		logval("writer_proceed.x", now.writer_proceed.x);
#endif
}
...
{	/* int i; */
	printf("global vars:\n");
		printf("	(struct readers_proceed)\n");
	printf("	int    x:	%d\n", now.readers_proceed.x);
		printf("	(struct writer_proceed)\n");
	printf("	int    x:	%d\n", now.writer_proceed.x);
}
void
